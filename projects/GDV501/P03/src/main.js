...
const cfg = {"n":"03","title":"Alchemist's Ledger","slug":"alchemists-ledger","repo":"GDV501-P03-alchemists-ledger","kind":"resource","deck":"Balance each order by using three visible conversion recipes.","generic":"Design resources as verbs; create puzzle constraints from conversion rules; teach through level sequence.","specific":"Use ore, glass, and signal as resources. Each level shows current stock, exact target stock, and the recipe on every action button so the puzzle teaches conversion, scarcity, and reversibility through play.","controls":"Click a recipe button. R resets the current ledger.","palette":["#f8ead2","#2b1d15","#a6542d","#28785f","#415d9c"],"id":"GDV501-P03"};
...
function convert(type){const l=resourceState(); let changed=false; if(type==="forge"&&l.a>=2){l.a-=2;l.b++;changed=true;} if(type==="focus"&&l.b>=1&&l.a>=1){l.b--;l.a--;l.c+=2;changed=true;} if(type==="reclaim"&&l.c>=1){l.c--;l.a+=2;changed=true;} if(changed){s.game.moves++; addLog(type+" recipe applied.");} else addLog("Not enough resources for "+type+"."); const g=l.goal; if(l.a===g.a&&l.b===g.b&&l.c===g.c){addLog("Ledger "+(s.game.level+1)+" balanced."); if(s.game.level===4)finish("All five ledgers balanced in "+s.game.moves+" moves."); else s.game.level++;}}
...
function renderFestival(){const g=s.game;g.gates.forEach(gt=>circle(gt.x,gt.y,26,gt.on?cfg.palette[2]:"rgba(24,32,28,.06)",gt.on?cfg.palette[2]:cfg.palette[1]));g.mirrors.forEach(m=>{ctx.save();ctx.translate(m.x,m.y);ctx.rotate(m.a*Math.PI/4);rect(-34,-5,68,10,cfg.palette[3]);ctx.restore();circle(m.x,m.y,36,"rgba(24,32,28,.045)","rgba(24,32,28,.16)");});ctx.strokeStyle=cfg.palette[3];ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(90,350);ctx.lineTo(1040,350);ctx.stroke();text("Clock "+Math.ceil(g.time)+"  Gates "+g.gates.filter(x=>x.on).length+"/3",430,92,18,cfg.palette[1],800);}
...
function click(x,y){ if(s.mode==="menu"){initGame(); return;} if(s.mode==="complete")return; if(cfg.kind==="resource"){["forge","focus","reclaim"].forEach((b,i)=>{const boxes=[[375,430,210,58],[610,430,250,58],[885,430,190,58]];const box=boxes[i];if(x>box[0]&&x<box[0]+box[2]&&y>box[1]&&y<box[1]+box[3])convert(b);});} if(cfg.kind==="bluff"){["probe","feint","commit"].forEach((b,i)=>{if(x>420+i*155&&x<550+i*155&&y>520&&y<574)bluff(b);});} if(cfg.kind==="choice"){[0,1,2].forEach(i=>{if(x>430&&x<730&&y>330+i*75&&y<384+i*75)choose(i);});} if(cfg.kind==="festival")clickFestival(x,y); if(cfg.kind==="timing")timingAction(); if(cfg.kind==="risk")bankRisk();}
