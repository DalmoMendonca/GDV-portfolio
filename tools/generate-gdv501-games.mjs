...
const games = [
  {
    ...
    slug: "chrono-aperture",
    repo: "GDV501-P01-chrono-aperture",
    kind: "timing",
    deck: "One button. Nine commitments. Three shrinking timing windows.",
    generic: "Design with extreme input constraint; create readable anticipation, action, and result states; tune difficulty through timing windows.",
    specific: "Press once as the moving needle enters the active aperture. Three tiers reduce the perfect window while preserving fair anticipation and clear early, good, perfect, and late feedback.",
    controls: "Space, click, or tap",
    palette: ["#122022", "#e9f1e7", "#58d6c9", "#f2bb5c", "#e45f46"],
  },
  {
    n: "02",
    title: "Hazard Dividend",
    slug: "hazard-dividend",
    repo: "GDV501-P02-hazard-dividend",
    kind: "risk",
    deck: "Every prize improves the score and worsens the board.",
    generic: "Model risk, reward, and opportunity cost; communicate danger without text dependency; balance short-term gain against long-term survival.",
    specific: "Collect valuable cores in a field of patrol hazards. Each core increases score, hazard speed, and map pressure. Exit early for a banking bonus or keep collecting for a riskier multiplier.",
    controls: "Arrow keys or WASD to move, Space to bank at the gate",
    palette: ["#111820", "#f3efe1", "#3ea6ff", "#f2c14e", "#e85d75"],
  },
  {
    n: "03",
    title: "Alchemist's Ledger",
    slug: "alchemists-ledger",
    repo: "GDV501-P03-alchemists-ledger",
    kind: "resource",
    deck: "Five puzzles about turning nouns into verbs.",
    generic: "Design resources as verbs; create puzzle constraints from conversion rules; teach through level sequence.",
    specific: "Use three resource types and two conversion actions across five authored levels. The sequence introduces conversion, scarcity, and goal checking without a tutorial paragraph.",
    controls: "Click conversion actions, R to reset level",
    palette: ["#17130f", "#f7efe2", "#c88442", "#65b891", "#6279b8"],
  },
  {
    n: "04",
    title: "The Quiet Bid",
    slug: "the-quiet-bid",
    repo: "GDV501-P04-the-quiet-bid",
    kind: "bluff",
    deck: "A tiny duel of tells, pressure, and private information.",
    generic: "Use uncertainty as a system; create readable tells and counterplay; design AI or local multiplayer decision moments.",
    specific: "Play five rounds against a readable bot. Probe, feint, or commit based on partial tells. Match recap reveals the hidden values and explains the counterplay.",
    controls: "Click Probe, Feint, or Commit",
    palette: ["#15151b", "#f2efe8", "#b9a7ff", "#efb15f", "#6fd0a8"],
  },
  {
    n: "05",
    title: "Kinetic Serif",
    slug: "kinetic-serif",
    repo: "GDV501-P05-kinetic-serif",
    kind: "movement",
    deck: "A five-room movement study built around a charged dash.",
    generic: "Make movement itself the primary mechanic; shape challenge through spatial affordances; tune acceleration, friction, jump, dash, glide, or equivalent properties.",
    specific: "Cross five compact rooms using one expressive movement verb: hold to charge, release to dash. Hazards, gates, and landing lanes teach control without combat or inventory.",
    controls: "Arrow keys or WASD to steer, hold/release Space to dash",
    palette: ["#10151d", "#f3f1e7", "#7bdff2", "#f2b5d4", "#b2f7b8"],
  },
  {
    n: "06",
    title: "Civic Fault Lines",
    slug: "civic-fault-lines",
    repo: "GDV501-P06-civic-fault-lines",
    kind: "choice",
    deck: "Choices alter the rules, not just the ending text.",
    generic: "Represent consequence in mechanics; distinguish cosmetic choice from systemic choice; build replayable branches.",
    specific: "Make three civic decisions. Each changes later costs, available actions, trust, and ending conditions. The final state is a mechanical consequence report, not a simple moral label.",
    controls: "Click one policy at each decision point",
    palette: ["#111816", "#f4efe1", "#4fb286", "#f4a261", "#d95d39"],
  },
  {
    n: "07",
    title: "Twin Lock Study",
    slug: "twin-lock-study",
    repo: "GDV501-P07-twin-lock-study",
    kind: "coop",
    deck: "Two bodies, asymmetric limits, one shared exit.",
    generic: "Design cooperation through asymmetric limits; create communication pressure; evaluate whether cooperation is required or optional.",
    specific: "Control two linked roles in four encounters. The heavy role anchors switches and blocks beams; the light role crosses gaps and collects keys. Either role alone is insufficient.",
    controls: "Tab switches roles, arrows/WASD move, Space uses role power",
    palette: ["#11131a", "#f2efe6", "#78a6ff", "#f7c873", "#ec6f86"],
  },
  {
    n: "08",
    title: "Director's Cut",
    slug: "directors-cut",
    repo: "GDV501-P08-directors-cut",
    kind: "adaptive",
    deck: "An arcade director watches pressure and changes the score.",
    generic: "Adapt challenge to player performance; measure player state without being intrusive; preserve dignity and agency when assistance is provided.",
    specific: "Dodge pressure waves while collecting signal marks. The director tracks hits, near misses, and collection rate, then changes spawn density, reward value, and recovery support with a transparent report.",
    controls: "Arrow keys or WASD to move",
    palette: ["#10141d", "#f3efe2", "#56cfe1", "#ffb703", "#fb7185"],
  },
  {
    n: "09",
    title: "Magnet Toybox",
    slug: "magnet-toybox",
    repo: "GDV501-P09-magnet-toybox",
    kind: "toy",
    deck: "A field toy becomes a scored collection game without losing play.",
    generic: "Differentiate toy, puzzle, and game; discover affordances through experimentation; add goals without killing expressive play.",
    specific: "Pull and push drifting particles with a magnetic cursor, then use the same expressive system to herd luminous motes into scoring rings before time expires.",
    controls: "Move mouse or drag, hold Space/click to attract, Shift to repel",
    palette: ["#101316", "#f1efe7", "#8bd3dd", "#f6c85f", "#ee6c4d"],
  },
  {
    n: "10",
    title: "Festival Signal",
    slug: "festival-signal",
    repo: "GDV501-P10-festival-signal",
    kind: "festival",
    deck: "A public-play station with instant onboarding and fast turnover.",
    generic: "Scope a complete experience for public play; create immediate onboarding; design for spectators and fast turnover.",
    specific: "Rotate signal mirrors to route light through three gates before the festival clock expires. The board is readable from a distance and can be completed or restarted in under ten minutes.",
    controls: "Click mirrors to rotate, R to restart",
    palette: ["#12140f", "#f4f0df", "#d9ed92", "#76c893", "#f28482"],
  },
];
...
function styles(game) {
  return `:root{color-scheme:dark;--bg:${game.palette[0]};--fg:${game.palette[1]}}*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#080b0f;color:var(--fg);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}body{overflow:hidden}.shell{min-height:100vh;display:grid;place-items:center;padding:18px;background:radial-gradient(circle at 50% 30%, color-mix(in srgb, ${game.palette[2]} 18%, transparent), transparent 35%),linear-gradient(135deg,#090c11,#151922 55%,#080a0d)}canvas{width:min(100%,1120px);height:auto;max-height:calc(100vh - 36px);aspect-ratio:1120/700;border:1px solid rgba(255,255,255,.18);box-shadow:0 28px 90px rgba(0,0,0,.45);background:var(--bg)}`;
}

function html(game) {
  return `<!doctype html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><meta name="description" content="${game.title}: ${game.deck}"/><title>GDV501-P${game.n} ${game.title}</title></head><body><main class="shell"><canvas id="game" width="1120" height="700" aria-label="${game.title} game canvas"></canvas></main><script type="module" src="/src/main.js"></script></body></html>`;
}

function packageJson(game) {
  return JSON.stringify({ name: game.repo.toLowerCase(), version: "1.0.0", private: true, type: "module", scripts: { dev: "vite --host 127.0.0.1", build: "vite build", preview: "vite preview --host 127.0.0.1" }, dependencies: { vite: "^7.2.4" } }, null, 2);
}

function readme(game) {
  return `# GDV501-P${game.n}: ${game.title}
...
for (const game of games) {
  ...
  await writeFile(path.join(dir, "README.md"), readme(game), "utf8");
  await writeFile(path.join(dir, "project.json"), `${projectJson(game)}\n`, "utf8");
  await writeFile(path.join(dir, "progress.md"), `Original prompt: Create and publish all ten GDV501 projects as professional exemplar games.\n\n## Progress\n\n- Generated ${game.title} as part of the GDV501 First Principles Arcade.\n- Includes canvas runtime, deterministic test hooks, Vite build, README, metadata, and Netlify config.\n\n## TODO\n\n- Build, test, publish, and update live links.\n`, "utf8");
}

const existing = await readFile(path.join(root, "src", "data", "projects.js"), "utf8");
let updated = existing;
for (const game of games) {
  const id = `GDV501-P${game.n}`;
  const re = new RegExp(`(\\{\\s*"id": "${id}"[\\s\\S]*?"netlifyUrl": ")[^"]*(")`, "m");
...
await writeFile(path.join(root, "src", "data", "projects.js"), updated, "utf8");
console.log("Generated GDV501 First Principles Arcade.");
